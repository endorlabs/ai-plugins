...
| Install the Cursor package | `.cursor-plugin/`, root `agents/`, and root `skills/` | Gemini extension files |
...
`assets/logo.svg` from the source repo. Sync `cursor-sdk/` for Python SDK
automation. Root `.mcp.json` and root `GEMINI.md` are support context, not
Cursor package output or an installable Gemini extension. The repo root must
not contain `gemini-extension.json`.
